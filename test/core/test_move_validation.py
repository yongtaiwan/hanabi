...
class TestMoveValidation(unittest.TestCase):
    ...
    def test_play_valid_card(self):
        ...
        if len(hand.cards) > 0:
            ...
            result = self.state._validateCardMove(0, move)
            # Should pass basic card move validation (actual play validity checked in _updatePlay)
    ...
    def test_discard_when_hints_full(self):
        ...
        new_common_view = CommonView(
            live_tokens=self.state.commonView.liveTokens,
            hint_tokens=max_hints,
            cards_to_draw=self.state.commonView.cardsToDraw,
            cards_discarded=self.state.commonView.cardsDiscarded,
            cards_played=self.state.commonView.cardsPlayed
        )

        new_state = GameState(
            start_position=self.state.startPosition,
            common_view=new_common_view,
            player_hands=self.state.playerHands,
            draw_deck_index=self.state.drawDeckIndex,
            turn_number=self.state.turnNumber,
            current_player=self.state.currentPlayer,
            turns_left=self.state.turnsLeft
        )

        hand = new_state.playerHands[0]
        if len(hand.cards) > 0:
            move = Discard(0)
            result = new_state._validateCardMove(0, move)
    ...
    def test_discard_when_hints_not_full(self):
        ...
        if common_view.hintTokens < self.state.settings.maxHintTokens:
            hand = self.state.playerHands[0]
            if len(hand.cards) > 0:
                move = Discard(0)
                result = self.state._validateCardMove(0, move)
    ...
    def test_hint_no_tokens(self):
        ...
        teammate_hand = new_state.playerHands[1]
        if len(teammate_hand.cards) > 0:
            ...
            result = new_state._validateHint(0, move)
    ...
    def test_hint_yourself(self):
        ...
        teammate_hand = self.state.playerHands[0]
        if len(teammate_hand.cards) > 0:
            card_color = teammate_hand.cards[0].color
            move = ColorHint(0, [0], card_color)  # Player 0 hinting themselves
            result = self.state._validateHint(0, move)
    ...
    def test_color_hint_must_include_all_matching_cards(self):
        ...
        player_hands = [self.state.playerHands[0], hand]
    ...
    def test_color_hint_invalid_card_index(self):
        ...
        teammate_hand = self.state.playerHands[1]
