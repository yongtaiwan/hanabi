...
from .game import GameSettings, CommonView, Hand, PlayerView
...
class ConsoleDisplay:
    ...
    def _colorize_message(self, message: str) -> str:
        ...
        # Colorize color names (standalone, not in Card()): RED, YELLOW, GREEN, BLUE, WHITE
        # Do this after card names to avoid double-coloring
        # Use a more specific pattern to avoid matching inside Card(COLOR, NUMBER)
        for color_name in ['RED', 'YELLOW', 'GREEN', 'BLUE', 'WHITE']:
            try:
                color_enum = Color[color_name]
                color_code = self._color_map.get(color_enum, Colors.BRIGHT_WHITE)
                # Match color name only if it's not part of "Card(COLOR, NUMBER)"
                # Look for color name followed by space and opening paren, or at end, or after colon
                result = re.sub(rf'(?<!Card\()\b{color_name}\b(?!,\s*\d+\))', 
                               f"{color_code}{color_name}{Colors.RESET}", 
                               result)
            except KeyError:
                pass
    ...
    def display_game_state(self, engine: GameEngine, player_index: int) -> None:
        ...
        self._display_move_history(engine, player_index)
        
        # Display tokens
        self._display_tokens(common_view, settings)
        
        # Display current score
        current_score = engine.getScore()
        ...
        # Display discard pile details
        self._display_discard_pile(engine.getDiscardPile())
        
        # Display draw deck count (make it more visible)
        deck_count = common_view.cardsToDraw
        if deck_count == 0:
            # Deck is exhausted - show warning
            if engine.isDeckExhausted() and engine.hasFinalTurnRemaining(player_index):
        ...
        player_view = engine.getPlayerView(player_index)
        ...
        player_hints = engine.getPlayerHints(player_index)
    ...
    def _display_move_history(self, engine: GameEngine, player_index: int) -> None:
        """Display the most recent moves from all other players."""
        # Get the most recent move from each other player
        recent_moves = engine.getRecentMovesFromOtherPlayers(player_index)
        
        if recent_moves:
            num_other_players = len(recent_moves)
            if num_other_players == 1:
                print(f"\n{Colors.BRIGHT_YELLOW}Other player's move:{Colors.RESET}")
            else:
                print(f"\n{Colors.BRIGHT_YELLOW}Other players' moves:{Colors.RESET}")
            
            for p_idx, move, msg in recent_moves:
                # Format player name
                player_name = f"{Colors.BRIGHT_CYAN}Player {p_idx + 1}{Colors.RESET}"
                
                # Convert Card(COLOR, NUMBER) to short notation (e.g., G1) for other players' moves
                # to keep text width consistent
                shortened_msg = self._shorten_card_notation(msg)
                
                # Colorize the message
                colorized_msg = self._colorize_message(shortened_msg)
                
                # Check if this is a hint given to the current player
                if isinstance(move, (ColorHint, NumberHint)) and move.teammate == player_index:
                    print(f"  {Colors.BRIGHT_GREEN}→ {player_name}: {colorized_msg}{Colors.RESET}")
                else:
                    print(f"  {Colors.BRIGHT_CYAN}→ {player_name}: {colorized_msg}{Colors.RESET}")
    
    def display_available_moves(self, engine: GameEngine, player_index: int) -> None:
        ...
        if hand.cards and common_view.hintTokens < engine.settings.maxHintTokens:
        ...
        elif hand.cards and common_view.hintTokens >= engine.settings.maxHintTokens:
    ...
    def display_game_end(self, engine: GameEngine) -> None:
        ...
        elif engine.settings.autoEndWhenNoPointsPossible and engine.isNoMorePointsPossible():
